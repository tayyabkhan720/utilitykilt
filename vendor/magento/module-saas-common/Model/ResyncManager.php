...
class ResyncManager
{
     ...
     *
     * @deprecated Kept for backwards compatibility; resync flows now use resetSubmittedData() directly.
     * @see \Magento\SaaSCommon\Model\ResyncManager::resetSubmittedData
    ...
    /**
     * Throw an exception if the configured feed indexer is currently running.
     *
     * @return void
     */
    ...
    public function partialResyncByIds(array $identifiers = [], string $identifierType = "default"): void
    {
        ...
        if (null === $mappedIdentifierType) {
            $message = 'CDE03-09 The \'' . $metadata->getFeedName() . '\' feed does not support partial resync by IDs,'
                . ' or an unsupported identifier type was specified.';
            $this->logger->warning($message);
            throw new \RuntimeException(
                'The \'' . $metadata->getFeedName() . '\' feed does not support partial resync by IDs,'
                . ' or an unsupported identifier type was specified.'
            );
        }
        $this->checkLock(function () use ($identifiers, $mappedIdentifierType, $metadata) {
            ...
            if ($idField !== null) {
                ...
                if (empty($idsToReindex)) {
                    $message = 'CDE03-10 There are no ' . $idField
                        . 's found to reindex for provided identifiers list: '
                        . implode(', ', $identifiers);
                    $this->logger->warning($message);
                    throw new \RuntimeException(
                        'There are no ' . $idField . 's found to reindex for provided identifiers list: '
                        . implode(', ', $identifiers)
                    );
                }
