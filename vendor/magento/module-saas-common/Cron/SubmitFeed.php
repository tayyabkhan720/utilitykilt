...
class SubmitFeed implements SubmitFeedInterface
{
    ...
    public function execute()
    {
        ...
        try {
            ...
            $this->logger->error(
                sprintf(
                    'CDE02-04 Cannot obtain feed metadata for feed name "%s". Sync terminated. Error: %s',
                    $this->feedName,
        ...
        for ($threadNumber = 1; $threadNumber <= $threadCount; $threadNumber++) {
            $userFunctions[] = function () use ($batchIterator, $metadata) {
                try {
                    ...
                    $this->logger->error(
                        sprintf(
                            'CDE02-06 Failed to retry feed items submission for feed %s. Error: %s',
                            $this->feedName,
                            $e->getMessage()
                        ),
                        ['exception' => $e]
        ...
        for ($threadNumber = 1; $threadNumber <= $threadCount; $threadNumber++) {
            $userFunctions[] = function () use ($batchIterator, $dateTimeFormat) {
                try {
                    ...
                    $this->logger->error(
                        sprintf(
                            'CDE02-05 Failed to submit feed batch for feed %s. Error: %s',
                            $this->feedName,
                            $e->getMessage()
                        ),
                        ['exception' => $e]
