...
class ElevatedRiskRejectedTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testUnholdOrder()
    {
        ...
        // Create refund on Stripe side
        $this->tests->stripe()->refunds->create(['charge' => $paymentIntent->latest_charge->id, 'reason' => 'fraudulent']);

        // Trigger charge.refunded to refund the order on Magento side
        $this->tests->event()->trigger("charge.refunded", $paymentIntent->latest_charge);

        // Give time for the handler to execute
        sleep(3);

        // Trigger the review.closed handler
