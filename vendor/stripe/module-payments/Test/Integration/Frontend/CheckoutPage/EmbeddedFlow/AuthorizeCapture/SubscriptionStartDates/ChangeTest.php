...
class ChangeTest extends \PHPUnit\Framework\TestCase
{
    ...
    /**
     * Tests upgrading a subscription that has a future start date (first_payment = on_start_date).
     * These subscriptions have latest_invoice = null because no billing has occurred yet.
     *
     * @magentoConfigFixture current_store payment/stripe_payments/payment_flow 0
     */
    public function testUpgrade()
    {
        // Use a day that is not today to ensure the start date is in the future
        $day = 10;
        if (date("d") == 10)
        {
            $day = 11;
        ...
        // For future start date subscriptions with first_payment = on_start_date,
        // there should be no latest_invoice and no charges
        $this->assertEmpty($subscription->latest_invoice, "Future start date subscription should have no latest_invoice");
        ...
        // No charges (start date is in the future)
        ...
        // Place the order — this should not fail with a TypeError about null $invoiceId
        ...
        // Still no charges (future start date)
        $charges = $this->tests->stripe()->charges->all(['customer' => $customerId]);
        $this->assertCount(0, $charges->data);

        // Trigger the first billing cycle by moving the billing anchor to now
        ...
        // Check the first billed order has the upgraded amount
