...
class ChangeTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function setUp(): void
    {
        ...
        $this->configProvider = $this->objectManager->get(\StripeIntegration\Payments\Model\Ui\ConfigProvider::class);
    }
    ...
    public function testUpgrade()
    {
        ...
        // Trial subscription should have no charges
        ...
        // Simulate the checkout page loading - this calls ConfigProvider::getConfig()
        // which should not reject the trialing subscription
        $config = $this->configProvider->getConfig();
        $subscriptionUpdateDetails = $config['payment']['stripe_payments']['subscriptionUpdateDetails'] ?? null;
        $this->assertNotEmpty($subscriptionUpdateDetails, "Subscription update details should be available for trialing subscriptions");
        $this->assertEquals("Never", (string)$subscriptionUpdateDetails['last_billed_label'], "Trial subscriptions should show 'Never' for last billed");
        ...
        // Still no charges during the trial
        $charges = $this->tests->stripe()->charges->all(['customer' => $customerId]);
        $this->assertCount(0, $charges->data);

        // End the trial — this triggers events and creates the first post-trial order
        $ordersCount = $this->tests->getOrdersCount();
        $this->tests->endTrialSubscription($subscription->id);
        $newOrdersCount = $this->tests->getOrdersCount();
        $this->assertEquals($ordersCount + 1, $newOrdersCount);

        // Check the first post-trial order has the upgraded amount
        $recurringOrder = $this->tests->getLastOrder();
        $this->assertEquals(31.65, $recurringOrder->getGrandTotal());

        // Process a second recurring billing cycle
        $customer = $this->tests->stripe()->customers->retrieve($customerId, [
            'expand' => ['subscriptions']
        ]);
    ...
    public function testDowngrade()
    {
        ...
        $this->quote->create()
            ->addProduct('simple-trial-monthly-subscription-product', 2)
            ->loginOpc()
            ->setPaymentMethod("SuccessCard");
        ...
        // Check the first post-trial order has the downgraded amount
        $recurringOrder = $this->tests->getLastOrder();
