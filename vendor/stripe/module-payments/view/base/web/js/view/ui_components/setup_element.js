    ...
    function (
        ...
        return Component.extend({
            ...
            initSetupElement: function(params)
            {
                ...
                var options = this.getStripeParam('elementsOptions');
                var elements = this.elements = stripe.stripeJs.elements(options);

                this.paymentElement = elements.create('payment', this.getPaymentElementCreateOptions());
            ...
            createPaymentMethod: function(onPaymentMethodCreated, onFail)
            {
                ...
                {
                    stripe.stripeJs.createConfirmationToken(confirmationTokenData).then(onPaymentMethodCreated, onFail);
                }, onFail);
            ...
            onPaymentMethodCreated: function(result)
            {
                ...
                {
                    var tokenId = result.confirmationToken ? result.confirmationToken.id : result.paymentMethod.id;
                    addPaymentMethodAction(tokenId, self.subscriptionId, function(response, status, xhr)
                    {
                            ...
                            try
                            {
                                ...
                                if (data.client_secret)
                                {
                                    stripe.authenticateCustomer(data.client_secret, function(err)
                                    {
                                        ...
                                        self.setNewPaymentMethods(newMethods, isDuplicate, data);
                                    });
                                }
                                else
                                {
