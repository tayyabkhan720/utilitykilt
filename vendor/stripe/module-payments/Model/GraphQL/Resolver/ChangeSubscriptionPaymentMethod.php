...
class ChangeSubscriptionPaymentMethod implements \Magento\Framework\GraphQl\Query\ResolverInterface
{
    private $api;
    private $serializer;
    ...
    public function resolve(
        ...
        if (empty($args['input']['subscription_id']))
            throw new GraphQlInputException(__("Please specify a subscription ID."));

        if (empty($args['input']['payment_method']))
            throw new GraphQlInputException(__("Please specify a payment method ID."));

        try {
            $result = $this->api->change_subscription_payment_method($args['input']['subscription_id'], $args['input']['payment_method']);
