...
class Collection extends \Magento\Framework\Model\ResourceModel\Db\Collection\AbstractCollection
{
    ...
    public function getSkippedChargeSucceededEvents($paymentIntentId)
    {
        $collection = $this
            ->addFieldToSelect('*')
            ->addFieldToFilter('is_processed', ['eq' => false])
            ->addFieldToFilter('payment_intent_id', ['eq' => $paymentIntentId])
            ->addFieldToFilter('event_type', ['eq' => 'charge.succeeded'])
            ->addFieldToFilter('last_error_status_code', ['eq' => 202]);
