...
class Data extends AbstractHelper
{
    ...
    protected function loadExtensionsData()
    {
        ...
        $curl->write(\Laminas\Http\Request::METHOD_GET, self::EXTENSION_LIST_URL . '?date=' . date('Y-m-d'), '1.0');
        $data = $curl->read();
        if ($data === false) {
            return false;
        }
    ...
    public function sendReviewData($data)
    {
        ...
        $curl->write(\Laminas\Http\Request::METHOD_POST, $this->getReviewUrl(), '1.1', [], $data);
        $result = $curl->read();
        if ($result === false) {
            return false;
        }
